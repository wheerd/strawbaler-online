...
| `@turf/area`, `@turf/kinks`, `@turf/boolean-valid`, `@turf/boolean-point-in-polygon`, `@turf/line-intersect` | Polygon validation and spatial predicates | Ensures perimeters avoid self-intersection and supports offset calculations; currently complemented by custom offset logic. |
